...
- **역방향 컬렉션을 만들지 않는다.** `Node.outgoing_edges` 같은 관계는 없다. 경로 탐색은 한 층의 Node·Edge 전체를 한 번에 읽어 메모리에서 처리하므로(`routing/`) 역방향 관계가 불필요하고, 있으면 N+1을 유발한다.
...
repositories / services / scripts.seed  ──►  models
