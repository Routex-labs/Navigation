...
3. **로깅**을 통일하고 예외를 로그로 남긴다 (`logging.py`)

비즈니스 규칙(경로 탐색, 좌표 변환)이나 HTTP 처리는 여기에 없다. "DB에 어떻게 붙는가"와
"무엇을 어떻게 로그로 남기는가"만 담당한다.
...
- **`NAV_LOG_LEVEL`은 기본 `INFO`다.** 요청 상태·에러는 항상 stdout으로 남는다.
  `DEBUG`로 내리면 우리 코드(`app.*`)의 상세 로그까지 본다. 설정은
  [`DEBUG_LOGGING.md`](../../DEBUG_LOGGING.md) 참고.
- **`NAV_WARM_EMBEDDING`은 기본 꺼짐이다.** 켜면 `main.create_app()`이 기동 직후
...
## `logging.py` — 로깅 설정과 예외 로깅

`main.create_app()`이 앱 생성 시 `configure_logging()`으로 포맷·레벨을 통일하고,
...
- **stdout 한 곳으로 통일.** uvicorn access(요청·상태 코드)·error(예외 트레이스백)와 우리
  코드(`app.*`) 로그를 모두 같은 포맷으로 stdout에 남긴다. Cloud Run 등이 자동 수집한다.
- **4xx/422는 이유와 함께.** 상태 코드 자체는 access 로그가 남기지만, `detail`·검증 실패 필드는
  예외 핸들러가 남긴다. 5xx는 `ERROR`, 4xx는 대부분 클라이언트 잘못이라 `WARNING`.
- **500 트레이스백은 uvicorn이 담당.** 미처리 예외는 uvicorn이 `uvicorn.error`로 남기므로
  따로 잡지 않는다 — `configure_logging()`이 그 로거도 stdout으로 통일한다.
- **각 모듈은 `logging.getLogger(__name__)`으로 찍는다.** 로거 이름이 `app.core.database`처럼
  패키지 경로를 그대로 써서, `app` 레벨을 한 번에 조절할 수 있다.

> 파일 기반 SQL/요청 진단 캡처(옛 `NAV_SQL_ECHO`/`NAV_HTTP_CAPTURE` → `app/sql/`·`app/args/`)는
> 제거됐다. 상세 로그가 필요하면 `NAV_LOG_LEVEL=DEBUG`로 실행한다.
...
logging.py    ──►  config.settings, fastapi/starlette 타입

routers / repositories  ──►  core.database.get_db, SessionLocal
main.create_app()       ──►  core.logging (설정·예외 핸들러)
...
| 상세 로그 보기 | `NAV_LOG_LEVEL=DEBUG`로 실행 후 stdout(또는 `backend-local.log`) |
