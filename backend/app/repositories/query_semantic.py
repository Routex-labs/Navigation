...
# `backend/scripts/evaluate_query_hybrid.py`로 재평가하고 이 파일·search-eval-set.md
...
#
# W9(2026-08-01)에서 intent를 소분류 규칙으로 확장했다(210건 → 368건, intent 2개 →
# 7개: 신발·식사에 카페·화장품·향수·의류·출구 추가). 재평가: 총점 23/29 그대로,
# FAISS 단독 22 그대로, 부정 4건 모두 no_match 유지. 유효 최하단이 0.507 → 0.522로
# 올라 임계값 0.50과의 마진은 오히려 넓어졌다 — 임계값 유지.
#
# **이 확장에서 배운 것: intent 이름이 소분류 라벨의 일부와 겹치면 그 단어가 문서에
# 두 번 들어가 나머지 단어를 밀어낸다.** `화장품` intent만 먼저 넣었을 때 소분류
# "화장품·향수" 매장의 문서가 "… 화장품·향수 화장품"이 되면서 "향수 보고 싶어"가
# 0.5 미만으로 떨어져 통과→실패로 회귀했다(23→22). `향수`를 같은 규칙으로 함께
# 넣어 무게를 되돌리고 나서야 23/29가 복구됐다. 복합 라벨("A·B")에 intent를 붙일
# 때는 A만 넣지 말고 B도 함께 볼 것.
#
# 답이 바뀐 1건은 통과 중이던 행("커피 마시고 싶어" 도조 커피→더커피). 경로도 함께
# 바뀌었다(light tier 2 → semantic) — "커피"가 이제 intent `카페`로 tier 1에 걸려
# 수십 건이 되므로 경량이 1건으로 확정하지 않는다. 상호에 그 글자가 든 한 곳으로
# 고정되던 동작이 사라진 것이라 의도한 결과다.
#
# W10(2026-08-02)에서 대분류 어휘를 재편했다(`식음료` → 음식점·카페·식품관, 편의시설의
# 영어 소분류 `restroom`·`facility` 등 → 한글). **문서 텍스트에 category·subcategory가
# 들어가므로 이건 인덱스 문서를 바꾸는 변경이다** — 위 규칙대로 재평가했다.
# 결과: 총점 23/29 그대로, FAISS 단독 22 그대로, 실패 6행도 그대로(질의·반환 매장·점수
# 전부 동일). 임계값 0.50 유지 근거 불변.
#
# 같은 차수에서 `식사`·`카페` intent 규칙을 소분류에서 **대분류**로 옮겼다
# (`subcategory=레스토랑` → `category=음식점`). 재편으로 두 집합이 정확히 같아졌고
# (57건·53건), 지도 필터 pill과 같은 축을 보게 하기 위해서다. 재평가 결과는 위와
# 동일 — 잡히는 매장 집합이 바뀌지 않으니 문서 텍스트도 바뀌지 않는다. 근거는
# store_facets.py 모듈 주석.
#
# W11(2026-08-09)에서 평가 세트를 29개 → 134개로 넓혔다(부정 4 → 20). 인덱스 문서는
# 건드리지 않았고, 기존 29건의 결과도 그대로다 — 바뀐 것은 **무엇으로 재는가**다.
# 부정을 늘리자 임계값 0.50이 실재 업종명 부정("수영장" 0.510)을 통과시키고 있다는 게
# 드러나 0.52로 올렸다(아래 상수 주석). 세트·판정 기준의 단일 출처는
# docs/backend/native/search-eval-set.md.
#
# 핵심 원칙:
...
#
# ## 다국어 모델 교체를 검토했다가 접었다 (2026-08-10)
#
# 영어로 친 질의("starbucks")가 한글 매장명을 못 찾는 문제 때문에
# `sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2`로 바꿔 재평가했다.
# 같은 세트(134건)를 같은 잣대로 채점한 결과는 **명백히 나빴다.**
#
# | | 최적 임계값에서 통과 | 오탐(negative) |
# |---|---:|---:|
# | ko-sroberta-multitask (현행) | 87/134 | **0** |
# | paraphrase-multilingual-MiniLM | 56/134 | 11 |
#
# 임계값을 그대로 두고 비교하면 불공정하므로(모델마다 유사도 분포가 다르다) 0.40~0.95를
# 훑어 각자의 최적점에서 비교했다. 그런데 다국어 모델은 **어느 임계값에서도 오탐이
# 0이 되지 않았다** — `aaaaaaaa`·`!!!!!!`·`노래방`·`주유소` 같은 무의미·무관 질의가
# 임의의 매장을 잡는다. 한국어 특화 모델을 작은 다국어 증류 모델로 바꾸면서 한국어
# 품질을 잃은 것이고, 세트 134건 중 131건이 한국어라 그 손실이 그대로 총점이 됐다.
#
# **영어 문제는 모델이 아니라 사전으로 풀었다.** 한국 매장명은 영어의 음차라 자동
# 변환 규칙이 없고(스타벅스 ≠ seutabeokseu), `resources/query_synonyms.json`에
# 영문 별칭을 넣는 편이 정확하고 오탐이 없다. 그 파일이 이 문제의 자리다.
#
# 다시 시도한다면 작은 증류 모델이 아니라 한국어 성능이 검증된 다국어 모델
# (bge-m3 급)이어야 하고, 임계값을 이 세트로 다시 잡아야 한다.
...
# thehyundai-seoul 실데이터로 튜닝(FAISS.md 8절·11절, 세트 설계는 search-eval-set.md).
#
# **0.50 → 0.52 (2026-08-09).** 평가 세트를 29개에서 134개로 넓히면서 부정 질의를 4개
# → 20개로 늘렸더니, 0.50에서 부정 2건이 매장을 반환했다:
#   "수영장" → 물 품 보 관 함 0.510, "qwertyuiop" → 빌보콰 0.506.
# 옛 부정 4건은 전부 키보드 난타(asdfqwerzxcv·ㅋㅋㅋㅋㅋ 등)라 0.43 언저리에 머물렀고,
# 그래서 0.50이 안전해 보였다. 실제 사용자가 던지는 부정 질의는 **이 건물에 없는 업종
# 이름**("수영장"·"찜질방"·"노래방")이고 그 점수대는 0.49~0.51다 — 난타보다 훨씬 높다.
# 세트가 작아서 못 보고 있었던 것이지 없던 문제가 아니다.
#
# 0.52는 확장 세트에서 부정 통과 0건이며, 대가는 정답 긍정 3건(80 → 77/114)이다.
# "틀린 매장 안내 > 다시 말해 주세요"라는 기존 정밀도 우선 원칙(11-1절)을 그대로 따른
# 선택이다. 유효 최하단 0.526과 최고 부정 0.510의 마진은 0.016.
#
# 상대 마진 규칙(11-1절 4번 "top1-top2 간격으로 무의미 질의를 거른다")은 같은 세트로
# 실측해 **기각**했다 — 부정 통과 0건 조건에서 최고가 39/114로 절대 임계값(77/114)보다
# 한참 나쁘다. 식당가처럼 같은 업종이 몰린 구역에서는 정상 질의도 top1과 top2가 붙기
# 때문이다. 재현: `python -m scripts.analyze_similarity_margin`.
#
# 데이터나 인덱스 문서를 바꾸면 `python -m scripts.evaluate_query_hybrid`로 다시 튜닝한다.
