...
def test_층_지도는_기존_JSON_구조를_유지한다(db_session):
    ...
    # 매장 카테고리(대분류/소분류)가 층 지도까지 전달된다. 미지정 매장은 None.
    by_name = {store["name"]: store for store in result["stores"]}
    assert by_name["가게A"]["category"] == "패션"
    assert by_name["가게A"]["subcategory"] == "여성패션"
    assert by_name["가게B"]["category"] is None
    assert by_name["가게B"]["subcategory"] is None
...
def test_층_그래프의_간선은_from_to_키를_사용한다(db_session):
    ...
    assert {"id", "from", "to", "length_m", "cost_m", "bidirectional"} <= set(result["edges"][0])
