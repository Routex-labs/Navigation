...
검증 기준:
    ...
    V2  1~3층은 에스컬레이터, 4층+는 엘리베이터가 더 싸다(층수 기반 수단 선택).
...
# V2 — 층수 기반 비용: 1~3층은 에스컬레이터, 4층+는 엘리베이터가 최단.
#
# 교차점은 두 상수(탑승시간 21초 / 대기 40초+승하차 10초)의 결과로 나오는 값이다.
# 상수를 바꾸면 이 테스트가 먼저 깨져서, 교차점이 조용히 이동하는 일을 막는다.
def test_비용모델이_층수에_따라_수단을_가른다():
    # 순수 비용 함수로 교차점을 고정한다. 에스컬 = 25×n, 엘리베 = 60 + 5×n.
    def esc_cost(n):
    ...
    assert esc_cost(3) <= elev_cost(3)  # 3층: 동률 — 보행 거리가 가른다
    assert elev_cost(4) < esc_cost(4)  # 4층+: 엘리베이터
    assert elev_cost(5) < esc_cost(5)
