...
///
/// ## 이것만으로 끝이 아니다 — 원인은 아직 확정되지 않았다
///
/// 위 15~36초를 만든 것이 무엇인지는 **아직 모른다.** 가능성이 최소한 둘이다.
///
///   1. 기기가 정말 그 주기로만 좌표를 만든다(위성 미확보 → 네트워크 측위).
///   2. **스트림이 죽었고, 일회성 조회만 화면을 떠받치고 있었다.**
///
/// 2번은 실제로 가능했다. 구독에 `onDone`이 없어서, 스트림이 에러 없이 닫히면
/// 앱이 그것을 알아채지 못한 채 죽은 구독을 들고 있었다
/// (`outdoor_map_screen.dart`의 `_subscribeToPositions`). 그 구멍은 막았고,
/// 어느 쪽이었는지는 진단 칩의 `스트림`/`직접`·`재시작N`으로 실기기에서 가른다
/// (`indoor_entry_gps.dart`의 `describeGpsBuildingJudgement`).
///
/// **그 결과가 나오기 전에는 이 파일을 지우지 않는다.** 1번이 원인이라면 이
/// 폴백이 유일하게 화면을 움직이는 장치다.
///
/// ## 어디서 깨지는가
///
