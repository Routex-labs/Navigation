...
/// 구간을 메우는 일회성 조회.
///
/// 좌표를 **어떻게 쓸지는 화면이 정한다.** 건물 안인지 판정하고, 마커를 그리고,
/// 경로 진행률을 갱신하는 일은 전부 화면 몫이다. 이 파일은 "좌표가 왔다"를
/// 끊기지 않게 배달하는 것까지만 책임진다.
///
/// 언제 켤지도 화면이 정한다([start]/[stop]). 실내에서도 켜 두는데, 야외 이탈
/// 판정의 유일한 입력이 GPS이기 때문이다 — 그 사정은 [gpsFixMaxAge] 주석에 있다.
///
/// ## 여기가 왜 따로 있어야 하나
///
/// 스트림이 죽는 방식이 셋이고, 셋 다 다르게 죽는다.
///
/// 1. **에러로 닫힘** — onError.
/// 2. **에러 없이 닫힘** — onDone. 네이티브가 EventChannel을 해제할 때다.
///    onError만 보고 있으면 구독 객체가 null이 아닌 채 남아, "이미 구독 중"으로
///    판단하고 영영 다시 열지 않는다.
/// 3. **열린 채 벙어리** — 둘 다 안 걸린다. geolocator 안드로이드가 포그라운드
///    서비스 바인딩 전에 구독하면 위치 요청을 걸지 않고 그냥 돌아선다.
///    [streamFirstFixTimeout] 감시가 유일한 탈출구다.
///
/// 셋을 한 곳에서 다루지 않으면 하나씩 빠뜨리게 된다. 실제로 2번과 3번이 차례로
/// 빠져 있었고, 실기기에서 "위치 갱신 버튼은 되는데 화면은 안 움직인다"로 나타났다.
library;
