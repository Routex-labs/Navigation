...
/// 구간을 메우는 일회성 조회.
///
/// 좌표를 **어떻게 쓸지는 화면이 정한다.** 이 파일은 "좌표가 왔다"를 끊기지 않게
/// 배달하는 것까지만 책임진다. 언제 켤지도 화면 몫이다([start]/[stop]).
///
/// 한 곳에 모아 둔 이유는 스트림이 **세 가지로 다르게 죽기** 때문이다 — 에러로
/// 닫힘(onError), 에러 없이 닫힘(onDone), 열린 채 벙어리
/// ([streamFirstFixTimeout]). 셋을 나눠 두면 하나씩 빠뜨린다.
///
/// 각 죽는 방식의 근거와 실기기 로그는 `docs/client/gps-stream-policy.md`.
library;
