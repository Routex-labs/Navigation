...
///
/// 한 곳에 모은 이유는 **정지가 즉시 끝나지 않기** 때문이다. `stopGuidance`는
/// 네이티브 pedometer를 flush·정지한 뒤에야 `idle`이 되는데, 그 사이의 `stopping`을
/// "돌고 있다"로 읽는 코드가 하나라도 있으면 세션 없이 앵커만 찍힌다 — 실기기에서
/// "나갔다 들어오면 위치가 굳는다"로 나타났다.
library;
