/// 건물의 **긴 축을 화면 세로에 맞춰** 지도를 돌리기 위한 기하 계산. 더현대 서울은
/// 정북에서 약 53도 돌아가 있어, 북쪽을 위로 두면 세로로 긴 폰 화면에 가로로 누운
/// 사각형을 담는 꼴이 된다.
///
/// 화면에서 떼어 둔 이유는 **부호 하나만 틀려도 조용히 90도씩 어긋나서**다.
library;
...
/// 건물을 감싸는 **최소 넓이 직사각형**. 축 정렬 상자와 다르다 — 더현대 서울은
/// 로컬 295×148 m가 정북 정렬로는 286×305 m가 된다. 이 상자의 긴 변이 건물의 긴 축.
typedef BuildingBox = ({
  /// 긴 변의 길이(m).
  double longSideM,
  ...
  /// **이 상자의 한가운데.** 카메라 목표점은 반드시 이 값이어야 한다 — 배율을
  /// 돌아간 상자로 계산하면서 중심만 정북 bbox에서 가져오면 그 차이만큼 도면이
  /// 밀린다(실측: B1이 여백 7%를 넘는 31 px 밀려 한쪽이 화면 끝에 붙었다).
  ll.LatLng center,
});
...
/// [footprint]를 감싸는 최소 넓이 직사각형. 점이 3개 미만이면 null.
///
/// 회전 캘리퍼스 대신 **각도를 훑는다** — 정의를 그대로 옮긴 것이라 부호·순회
/// 실수로 90도 어긋날 여지가 없고, 수십 점 규모에서 비용도 문제가 안 된다.
///
/// [covering]을 주면 **각도는 [footprint]로 정하고 크기·중심만 넓힌다.** 매장
/// 폴리곤과 POI는 층 외곽선 밖으로 나갈 수 있지만(1F는 위아래로 12·19 m), 각도까지
/// [covering]으로 정하면 **축이 층마다 흔들려** 층을 바꿀 때마다 지도가 회전한다.
...
/// 긴 축이 화면 **세로**로 서도록 하는 카메라 bearing(0~360 미만). MapLibre의
/// bearing은 "화면 위쪽에 오는 나침반 방향"이라 긴 축 방위각을 그대로 주면 된다.
///
/// 축은 선이라 방위각과 +180 둘 다 세로로 세운다. **지금 카메라에서 덜 도는 쪽**을
/// 고른다 — 굳이 170도를 돌면 지도가 한 바퀴 도는 것처럼 보인다.
...
/// 경로 폴리라인을 감싸는 최소 넓이 상자. [minAreaBoxFor]와 같되 **퇴화한 입력을
/// 견딘다** — 점이 둘뿐이면 중점을 끼워 3점으로 만들고, 일직선이면 짧은 변이 0으로
/// 수렴해 zoom이 발산하므로 두 변 모두 [minSideM]으로 받친다.
///
/// **긴 축 방위각은 손대지 않는다** — 그 값이 곧 카메라를 세우는 기준이다.
...
/// [origin]에서 방위각 [azimuthDeg] 쪽으로 [meters]만큼 떨어진 지점. 지도가 돌아가
/// 있으면 "화면 위"가 북쪽이 아니라서 필요하다.
