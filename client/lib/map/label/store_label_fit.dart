...
import '../camera/zoom_math.dart';

/// 매장명 라벨을 **매장 폴리곤 크기에 맞춰** 재는 계산기.
///
/// 크기를 px이 아니라 **"1em이 몇 미터인가"**([kStoreLabelEmMetersProperty])로
/// 계산해 feature마다 싣는다 — 미터는 zoom과 무관하다.
///
/// 값의 근거·실측 표·알고 두는 한계 넷은 `docs/client/map-style-rules.md` 5절.
...
/// 읽을 수 있는 하한(px). 이보다 작아야 하는 매장은 **넘침을 감수하고** 이 크기로
/// 그린다 — 라벨을 숨기는 대신 유지하기로 한 결정이다.
const kStoreLabelMinPx = 12.0;

/// 상한(px). 큰 매장에서 글자가 도면을 삼키지 않게 막는다.
///
/// 하한과 함께 **한 번 반대 방향으로 튜닝했던 밴드다**(9~18 → 9~14 → 12~17).
/// 세 번의 이유가 서로 달라 근거를 지우면 안 된다 — map-style-rules.md 5절.
const kStoreLabelMaxPx = 17.0;

/// MapLibre가 쓰는 줄 높이(em). shaping에서 `lineHeight = 1.2 * fontSize`다.
const kStoreLabelLineHeightEm = 1.2;

/// 몇 줄까지 접어 볼지. **3에서 2로 내렸다** — 3줄이면 `조 말론 런던`이 글자 단위로
/// 흩어져 세 조각이 각각 다른 매장 이름처럼 읽혔다. [kStoreLabelMinLineEm]과 짝이다.
const kStoreLabelMaxLines = 2;

/// 접었을 때 한 줄의 **최소 폭(em)**. 전각 두 글자다.
///
/// 줄 수만 제한하면 「르 라보」가 `르` / `라보`가 되어 첫 줄 한 글자가 이웃 이름의
...
///
/// `전체 폭 / 줄 수`는 답이 아니다 — 그건 평균이지 탐욕 배치가 달성하는 폭이
/// 아니다. 답이 될 수 있는 폭은 「연속한 덩어리들의 합」뿐이라 그것만 훑는다.
/// 이름이 아주 길면([_wrapScanChunkLimit] 초과) 옛 근사식으로 물러난다.
...
/// [name]이 박스 안에 들어가는 최대 글자 크기. 줄 수를 바꿔 가며 다 재본다 —
/// 줄을 늘리면 가로 예산은 넉넉해지고 세로는 빠듯해져 유리한 쪽을 미리 못 정한다.
/// 박스가 없으면 `emMeters: 0`이고 [kStoreLabelMinPx]로 그려진다.
...
/// 매장 폴리곤이 **화면에서** 차지하는 가로/세로(m). 위경도 bbox가 아니라 카메라
/// [bearingDeg]로 돌린 좌표계에서 잰다 — 안 돌리면 비스듬한 매장의 박스가 실제보다
/// 크게 잡힌다.
...
///
/// **zoom 보간을 최상위에 둔다**(map-style-rules.md 0절). 그래서 feature 계산은
/// stop 값 쪽에 넣는다. stop을 1레벨 간격으로 촘촘히 두는 이유는 clamp다 — 양
/// 끝점만 두면 clamp 구간에서 값이 어긋난다.
...
/// 라벨 GeoJSON 소스에 걸 `maxzoom`. **스펙 상한이다.** 기본값 18에서는 오버줌
/// 구간에 심볼의 **아이콘만** 통째로 빠졌고, 값을 올릴 때마다 소실 지점이 한
/// 단계씩 밀렸다(실측 표: map-style-rules.md 5절).
