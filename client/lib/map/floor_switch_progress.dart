...
///
/// 덮개를 씌우지 않는다 — **이전 층 도면을 계속 보여 주다가** 새 층 타일이 실제로
/// 도착한 뒤 크로스페이드한다. 흰 화면은 어느 순간에도 없다.
///
/// "타일이 도착했다"는 고정 딜레이가 아니라 **실제 로드 신호**다(화면이
/// `querySourceFeatures`로 폴링). 주차구역이 수백 개라 수 초 걸리는 무거운 층에서도
/// 이전 도면이 끝까지 유지되는 근거다. maplibre_gl의 `waitUntilMapTilesAreLoaded`는
/// native에서 no-op이라 못 쓴다.
///
/// 여기 있는 것은 타이밍 **정책**과 모티프를 언제 띄우고 거두는가의 **판단**뿐이다.
/// 시간 축 위의 상태 기계라 위젯 트리 없이 검증해야 경우의 수(빠른 완료·연타·늦은
/// 완료)를 다 돌려볼 수 있다. 연출 전체의 근거는
/// `docs/client/camera-choreography-plan.md` 4.5.
library;
