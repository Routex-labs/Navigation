...
///
/// 나머지 이름을 내리면 남는 글자가 곧 답이 된다. **아이콘은 남긴다** — 이름까지
/// 지우면 그 폴리곤이 매장인지 통로인지 알 수 없어 도면이 맥락을 잃는다.
///
/// 레이어를 둘로 나누지 않고 `case`를 쓴다. `case`의 인자는 배열이 아니라 불리언
/// 표현식이라 이 파일이 경계하는 "인자 자리에 데이터 배열" 형태와 다르다. 조건에
/// [categoryHighlightFilter]를 그대로 넣어 **강조되는 매장과 이름이 남는 매장이
/// 정의상 같은 집합**이 되게 한다.
