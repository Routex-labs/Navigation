...
///
/// 정규화는 **질의와 이름의 매칭용 사본에만** 적용하므로 원본 이름은 그대로
/// 남는다. 같은 이름은 한 줄로 묶되 **여기서 대표를 고르지 않는다** — 어느 층을
/// 적을지는 거리를 아는 쪽만 정할 수 있다([StoreSuggestion] 주석).
///
/// 우선순위 규칙, 정규화 범위, IME 조합 중 자모 처리의 근거는
/// `docs/client/search-input-assist.md` K절이 단일 출처다.
