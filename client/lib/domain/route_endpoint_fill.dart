...
/// 벌어진 거리에 따라 셋으로 갈린다.
///
/// 1. [_minGapMeters] 미만 — 같은 점이다. 그대로 둔다.
/// 2. [_snapToleranceMeters] 이하 — 도착은 했고 마지막 몇 걸음이 비었다.
///    도착점을 뒤에 붙여 선을 잇는다.
/// 3. 그보다 멀다 — **TMAP이 우리 도착점에 닿지 못했다**([_snapToleranceMeters]
///    주석). 받은 선의 뒷부분은 우리 문이 아니라 엉뚱한 곳으로 가는 우회로이므로,
///    직선으로 가는 편이 확실히 짧아지는 첫 지점에서 끊고 도착점까지 직선으로
///    잇는다. 그런 지점이 없으면(직선도 길거나 별 이득이 없으면) 2번처럼 잇기만
///    하고, [_maxGapMeters]를 넘으면 그것도 하지 않는다.
///
/// [route]가 null이거나 좌표가 부족하면 그대로 돌려준다.
///
/// 2번에서는 거리·시간을 **건드리지 않는다** — 메우는 구간이 수십 미터라 총계에
/// 의미 있는 차이를 만들지 않고, 여기서 더하기 시작하면 화면에 적히는 숫자의
/// 출처가 둘로 갈린다. 3번은 선을 잘라 내 기하가 실제로 달라지므로 새 기하에
/// 맞춰 다시 계산한다(같은 보행 속도로 환산).
