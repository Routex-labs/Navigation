...
/// | 상황 | 기대 |
/// |---|---|
/// | 보통 | 거리 오름차순, 자기 자신은 빠짐 |
/// | 입구 노드가 없는 매장 | 뺀다(직선거리로 대신하지 않는다) |
/// | 도달 불가(그래프가 끊김) | 뺀다 |
/// | 주차·에스컬레이터·엘리베이터 | 뺀다(`kind != store`) |
/// | 화장실 등 시설 | 뺀다 — 제목이 "근처 매장"이다 |
/// | 같은 거리 두 매장 | 이름순으로 고정(호출마다 순서가 바뀌지 않게) |
/// | 같은 입구 노드를 쓰는 매장 | 거리 0으로 남는다 — 실제로 바로 옆이다 |
/// | 결과가 limit보다 적음 | 있는 만큼만 |
/// | 결과가 0 | 빈 목록(호출부가 섹션을 통째로 생략) |
library;
