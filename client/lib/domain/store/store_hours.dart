...
///
/// **판정 문자열을 저장하지 않는 것이 이 파일의 존재 이유다** — 서버가 "영업 중"을
/// 내려보내면 응답을 만든 순간부터 틀리기 시작하고 ETag 캐시로 더 오래 살아남는다.
/// [DateTime.now]도 인자로 받는다(안에서 읽으면 테스트가 시각에 따라 흔들린다).
///
...
///
/// **90일 = 약 한 분기.** 영업시간이 바뀌는 사건(조정·시즌 연장·리뉴얼)이 대체로
/// 분기 단위로 온다. 180일이면 연장→단축 전환을 통째로 한 번 놓치고, 30일이면
/// 539건을 매달 확인해야 해서 규칙이 안 지켜진다.
///
/// **요일 표까지 지우지는 않는다** — 낡아도 대체로 맞고, 지우면 "정보가 없는 앱"이
/// 된다. 거두는 것은 헛걸음을 만드는 **지금 시각 판정** 한 줄뿐이다.
...
/// [from]부터 [days]일 동안의 영업 구간을 절대 시각 쌍으로 편다. 자정을 넘긴 구간을
/// 여기서 한 번에 펴 두면 이후 계산에 자정이라는 개념이 남지 않는다.
///
/// **맞닿은 구간은 하나로 잇는다** — 종일 영업이 자정마다 "닫았다 연다"가 된다.
