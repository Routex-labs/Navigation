...
///
/// 서버의 `reason`은 질의 전체의 근거와 그 매장의 근거를 한 문장씩 이어 붙인 것이라
/// (`query_search._reason`), 목록으로 놓으면 앞 문장이 행마다 똑같다. 그건 목록
/// 머리말이 이미 말하고 있어 정보량 0으로 **층이 있던 자리**를 밀어냈다.
///
/// **서버 쪽에서 빼지 않는다.** `reason`은 한 건만 떼어 봐도 근거가 서야 한다는
/// 계약이다(질문 축만 담으면 "신발"을 물은 사용자에게 "명품 스타일 매장이에요"만
/// 남는다). 여러 행을 함께 놓고 볼 때 무엇이 겹치는지는 **화면만 아는 사실**이다.
library;
