...
/// ## 문은 데이터에 이미 있다
///
/// 더현대 서울 1층에는 `편의시설/교통` 소분류로 "출구" 5개가 들어 있고, 전부
/// `entrance_node_id`가 채워져 있다. 즉 **문마다 야외 좌표와 실내 그래프 노드가
/// 한 쌍으로 존재한다** — 야외 경로는 좌표에서 끝내고 실내 경로는 노드에서
/// 시작하면 두 구간이 정확히 맞물린다.
///
/// 같은 소분류에 지하철역 마커("9호선, 5호선 여의도역" 등)도 들어 있지만 그쪽은
/// `entrance_node_id`가 비어 있다. 그래서 [groundEntrancesFrom]은 소분류만이
/// 아니라 **노드가 있는지**로 한 번 더 거른다 — 노드 없는 지점을 문으로 받으면
/// 실내 구간의 시작점을 만들 수 없어 그 자리에서 경로가 끊긴다.
library;
