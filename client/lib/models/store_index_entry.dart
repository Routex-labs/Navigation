...
///
/// **좌표가 없는 것이 이 모델의 요점이다.** 같은 매장을 담는
/// [PoiSearchResult](poi_search_result.dart)는 지도에 찍을 실좌표를 들고 있지만,
/// 후보 목록은 이름과 층 라벨만 그리므로 좌표를 한 번도 쓰지 않는다. 1640건
/// 전부에 좌표를 붙이면 응답이 333KB에서 최대 1.6배가 된다(측정:
/// `docs/client/search-input-assist.md` K절).
///
/// 그래서 **후보를 고르면 좌표를 들고 바로 이동하지 않는다.** 그 이름으로 검색을
/// 다시 돌려(`SearchPanel.onQueryPicked`) 기존 경량 매칭이 좌표까지 갖춘 결과
/// 1건을 돌려주게 한다. 네이버 자동완성이 제안을 누르면 그 말로 검색이 실행되는
/// 것과 같은 흐름이고, 좌표를 두 벌로 들고 다닐 이유가 사라진다.
