...
/// 원본 데이터가 규칙을 가진다 — `ES1-UP(TO3F)`는 3F로 올라가는 탑승 지점,
/// `ES1-UP(FR2F)`는 2F에서 올라온 도착 지점(그룹 라벨이 빈 노드도 있다).
///
/// **왜 이름을 쓰는가.** 전이 간선은 8 m 반경 근접 매칭이라, 한 랜딩에 1.5 m로 붙은
/// 탑승·도착 노드가 "탑승↔탑승"으로 이어질 수 있다. 라우팅 결과는 멀쩡해 보여서
/// 드러나지 않는다 — 도착 위치에는 **이름이 유일하게 정확한 근거**다.
library;
