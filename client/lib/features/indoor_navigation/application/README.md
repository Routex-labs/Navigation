...
- 덮개 가운데 카드의 점은 **지도 마커 그 자체**다. 생김새를 현재 위치 마커와 맞추고
  (`kLocationMarkerCoreRadiusPx` 등 같은 상수를 본다), 위치도 마커와 **같은 진행률**로
  움직인다. 덮개가 마커를 가리는 것이 아니라 데려가서 새 층에 내려놓는 그림이라, 사용자가
  보는 것은 처음부터 끝까지 같은 점 하나다. 그래서 반복 재생을 없앴다 — 1.6초 왕복을 무한
  반복하던 시절에는 덮개가 3초를 넘기자 같은 장면이 두 번 재생돼 "지금 어디쯤"이 오히려
  안 읽혔다. 캡션도 가는 방향 쪽에 붙인다.
