...
하차 직후 남는 오차는 평활 지연 한 박자(창 3초 × 수직 속도 ≈ 0.2~0.4m)뿐이고, 문턱
(1.2m)보다 한참 낮다. 그 잔차가 반대 방향 후보를 열지도 않는다 — 평활값이 아직 따라
올라오는 중이라 램프 부호가 하강이 아니다.
...
| `verticalMotionDetected` | 같은 방향 수직 속도 0.12 m/s 이상이 2샘플 연속. **노드 허가가 없으면** 누적 고도 0.8m 이상 + 기기 움직임 신호를 더 요구한다 | 위치에 반영하는 걸음을 멈추고, 마커를 **반드시** 어딘가에 고정하고, 화면을 덮개로 덮는다 |
| `midpointReached` | 누적 Δ ≥ 1.2m + ramp 일관성(0.45m/2.5s) + 유효 탑승/도착 노드 | 목적 층 도면·경로를 연다 |
| `landed` | 누적 Δ ≥ 2.2m + 수직 속도 감소(+원시 움직임) | 새 anchor 확정, 걸음 적용 재개 |

**노드는 "층을 바꿔도 되는가"의 허가이지 "지금 수직으로 움직이는가"의 근거가 아니다.**
실측에서 랜딩의 보정 위치가 12m까지 어긋나 허가가 안 걸리는 일이 흔했고, 그때 사용자는 이미
에스컬레이터 위인데 마커만 계속 걸어갔다. 그래서 허가가 없어도 걸음 정지와 화면 덮개까지는
간다 — 대신 근거를 둘 더 요구한다. 누적 고도가 일상적 변동(문 여닫힘·HVAC, 평활 뒤 ±0.3m)을
넘는 **0.8m 이상**(`earlyVerticalDeltaM`, 반 층에도 못 미치는 값이라 "층이 바뀌었다"가 아니라
"수직으로 실려 가는 중"만 말한다)이고, 기기가 실제로 움직이는 중이라는 신호가 있을 때다.
후자가 없으면 책상 위 폰의 기압 드리프트로도 화면이 덮인다. 이렇게 열린 단계는 하차를 확정할
노드가 없으므로, 수직 이동이 멎는 즉시(2샘플) 접는다 — 40초 타임아웃을 기다리면 내려서 걷기
시작한 사용자에게 앱이 죽은 것과 같다. **층 교체는 여전히 노드 허가와 `minDeltaM`을 요구한다**
(되돌릴 수 없는 쪽은 근거를 그대로 둔다).
...
- `midpointReached`에서 마커는 새 층 도착 노드에 고정된다. 마커가 안 움직이는 이유를
  알려주지 않으면 "앱이 멈췄다"로 읽히므로 배너 문구가 단계별로 바뀐다. 확정 뒤에는 잠깐
  "도착" 상태로 바뀐다. 같은 사실을 말하는 별도 토스트는 띄우지 않는다.
...
- **덮개는 걸음이 멈추는 순간부터 하차까지 덮는다.** 한때 도면 교체 1.6초만 덮었는데, 그
  앞뒤로 걸음이 멈춘 마커가 지도 위에 굳은 채 서 있어 "위치가 고장 났다"로 읽혔다. 덮개가
  올라가고 내려가는 시점은 걸음 pause/resume과 **같은 사건**이다. 그 구간에 사용자가 지도
  에서 할 수 있는 일도 없다(위치가 갱신되지 않는다). 페이드는 느리다(진입 520ms / 해제
  700ms) — 빠르면 전환이 아니라 깜빡임으로 읽힌다.
...
- 덮개 뒤에서 **마커도 계속 흐른다**(`EscalatorGlide`). 탑승 노드에서 하차 노드까지
  2.4초에 걸쳐 이동한다. 이 구간은 수평 위치를 측정할 수 없는(걸음 정지 + 좌표계 교체) 구간
  이라 측정값이 아니라 **양 끝을 아는 보간**이며, 하차가 확정되면 실제 앵커가 도착 노드에
  잡히므로 연출이 끝나는 자리와 실제 위치가 어긋나지 않는다. 양 끝 중 하나라도 모르면 걸지
  않는다.
- 카메라는 같은 시간 동안 하차 지점으로 옮기고, **각도를 에스컬레이터에서 내리는 방향**에
  맞춘다(`escalatorExitBearingDeg`, 탑승 노드 → 하차 노드 방위각). 예전에는 새 층 경로의 긴
  축에 맞췄는데, 내리는 순간 화면은 이미 "앞으로 갈 방향"인데 몸은 에스컬레이터 정면을 보고
  있어서 어느 쪽으로 틀어야 하는지 화면에서 읽을 수 없었다. 두 노드가 3m 안쪽으로 겹쳐 있는
  도면에서는 방향을 단정하지 않고 카메라 각도를 그대로 둔다.
