...
| `verticalMotionDetected` | 같은 방향 수직 속도 0.12 m/s 이상이 2샘플 연속 | 위치에 반영하는 걸음을 멈추고, 마커를 **반드시** 어딘가에 고정한다(아래) |
...
배너를 이르게 띄우는 이유는 되돌리기 비용이 거의 없기 때문이다(문구가 사라질 뿐이다).
...
- `midpointReached`에서 마커는 새 층 도착 노드에 고정된다. 마커가 안 움직이는 이유를
  ...
  "도착" 상태로 바뀌며 되돌리기를 제공한다. 같은 사실을 말하는 별도 토스트는 띄우지 않는다.
- 배너는 이 화면이 아니라 **`MapShellScreen` root Stack**이 그린다. 검색창·카테고리 줄·
  하단 바가 셸의 형제라, 지도 안에서 그리면 상단 UI가 최악 높이일 때 뒤에 깔린다.
- **화면을 덮는 스크림은 없다.** 한때 도면 교체 구간을 불투명 스크림으로 덮었는데(진입
  520ms → 불투명 400ms → 해제 700ms), 실기기에서 그것은 전환이 아니라 **깜빡임**으로
  보였다 — 지도가 하얗게 한 번 날아가고 그 사이 마커가 사라졌다 다른 자리에 나타난다.
  지금은 사람이 층 chip을 눌렀을 때와 같은 크로스페이드를 쓰되 페이드만 두 배 느리게
  준다(`floorSwitchGuidedCrossfadeDuration`, 600ms). 이전 층 도면은 새 층 타일이 실제로
  도착할 때까지 남아 있다.
- 도면이 바뀌는 동안 **마커는 계속 흐른다**(`EscalatorGlide`). 탑승 노드에서 하차 노드까지
