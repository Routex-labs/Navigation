...
| `verticalMotionDetected` | 수직 속도 0.12 m/s가 2샘플 연속(1차, 조용함) **그리고** 탑승점 3m 근접 또는 누적 고도 1.8m 초과(2차) | 위치에 반영하는 걸음을 멈추고, 마커를 **반드시** 어딘가에 고정한다 |
...
**2차에서만** 걸음을 멈추고 마커를 세우고 덮개를 올린다. 올라가는 길이 둘이다.

1. **탑승점 3m 근접**(`boardingApproachRadiusM`). 허가 반경(6m, 경로가 지목하면 16m)으로는
   부족하다 — 그건 "층을 바꿔도 되는가"의 허가일 뿐이고, 그 거리에서 마커를 세우면 점만 저 앞
   에스컬레이터에 붙어 멈춘 화면이 된다.
2. **누적 고도 1.8m 초과**(`visibleVerticalDeltaM`) + 기기가 움직이는 중이라는 신호. 층고의
   3분의 1쯤이라 계단 몇 칸이나 기상 드리프트로는 안 나오고, 반 층에는 못 미쳐 하차 전에
   잡힌다. 실측에서 랜딩의 보정 위치가 12m까지 어긋나 허가가 안 걸리던 사례를 이 갈래가 받는다.
   움직임 신호가 없으면 책상 위 폰의 기압 드리프트로도 화면이 덮인다.

2번으로 열렸는데 노드를 못 골랐으면 하차를 확정할 수단이 없다. 그래서 수직 이동이 멎는 즉시
...
- **덮개는 도면이 갈리는 앞뒤만 덮는다**(진입 520ms → 유지 2초 → 해제 700ms, 약 3.2초).
