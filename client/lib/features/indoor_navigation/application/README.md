...
- **방향 선택이 생기는 노드 앞뒤 약 3.5m는 회전 허용 구간이다.** 사람은 코너 안쪽을 잘라
  ...
  돌아가 실제 이탈을 숨기지 않는다.
- 결과는 방향을 **두 개** 내보낸다. `correctedHeadingDeg`는 잠긴 진행 방향의 edge 접선이고,
  `deviceHeadingDeg`는 센서 heading + 학습한 bias, 즉 사용자가 실제로 바라보는 방향이다.
  하나로 합치면 안 된다 — edge 접선은 걸음이 있어야 갱신되므로 직선 복도에서 제자리
  회전하면 화면의 방향이 얼어붙는다. 마커 원뿔·카메라 정렬과 "지금 어디를 보고 있나"를
  묻는 판단(역주행·후퇴 허용)은 `deviceHeadingDeg`를 쓴다.
...
| 단계 | 근거 | 하는 일 |
|---|---|---|
| `boardingDetected` | 활성 경로의 탑승점 3m 이내 + 서로 다른 걸음 갱신 2회 이상 연속 접근 | 배너만 띄운다 |
| `verticalMotionDetected` | 같은 방향 수직 속도 0.12 m/s 이상이 2샘플 연속 | 위치에 반영하는 걸음만 멈춘다 |
| `midpointReached` | 기존 조건 그대로(누적 Δ ≥ 1.8m + ramp 일관성 + 유효 탑승/도착 노드) | 목적 층 도면·경로를 연다 |
...
- 배너와 층 교체 베일은 이 화면이 아니라 **`MapShellScreen` root Stack**이 그린다. 검색창·
...
- **후퇴는 방향으로 판단한다.** 빔 1등 재배치나 초록·주황 보폭 차이로 진행거리가 2m 넘게
  줄었는데 사용자가 여전히 경로 방향을 보고 있으면(방위 오차 100° 미만) 그 갱신을 보류한다.
  걸음 수는 근거가 못 된다 — pedometer는 방향을 모르므로 **앞으로 걸은 걸음이 뒤로 갈 여유로
  계산됐다.** 안내 중 마커는 이 진행점의 투영 좌표를 쓰므로, 이 억제가 곧 "마커가 뒤로
  튀지 않는다"와 같은 말이다.
- 진행 계산의 `headingDeg`도 간선 방위가 아니라 사용자가 향한 방향을 쓴다. "반대 방향입니다"와
  후퇴 판단이 둘 다 "지금 어디를 보고 있나"를 묻는데, 간선 방위를 쓰면 빔이 고른 진행 부호가
