...
| `verticalMotionDetected` | 같은 방향 수직 속도 0.12 m/s 이상이 2샘플 연속 | 위치에 반영하는 걸음만 멈추고, 화면을 반투명 스크림으로 덮는다 |
...
탑승점 고정은 **경로가 그 에스컬레이터를 타라고 했을 때만** 한다(현재 층 세그먼트의
`transferModeToNext == 'escalator'` 이고 `transferFromNodeId`가 판정기가 고른 탑승 노드와
같을 때). 근처를 지나가는 것만으로 고정하면 그냥 걷는 사용자의 마커가 멈춰 선다. 고정 중에는
...
- 배너와 층 전환 스크림은 이 화면이 아니라 **`MapShellScreen` root Stack**이 그린다. 검색창·
  카테고리 줄·하단 바가 셸의 형제라, 지도 안에서 그리면 상단 UI가 최악 높이일 때 뒤에 깔린다.
- 스크림은 **도면을 실제로 갈아 끼우는 구간에만** 뜬다(진입 520ms → 완전 불투명 400ms →
  해제 700ms, 약 1.6초). 탑승~하차 구간 전체를 반투명으로 덮어 본 적이 있는데, 그 구간은
  수십 초라 "전환 중"이 아니라 "화면이 계속 덮여 있다"로 읽혔다. 덮개는 도면이 바뀌는
  **사건**을 알리고, 탑승 중이라는 **상태**는 배너가 말한다. 페이드가 느린 이유는 빠른
  페이드가 "깜빡"으로만 보여서 층이 바뀐 줄 모른 채 다른 층 도면을 읽게 되기 때문이다.
  거의 다 가려진 동안에만 뒤쪽 입력을 막고, 걷히는 동안에는 통과시킨다. 스크림 카드에는
  진행 방향으로 흐르는 chevron 애니메이션을 둬서 올라가는지 내려가는지가 한눈에 읽히게
  한다. 스크림이 올라오면 배너는 접는다(같은 문장이 두 벌로 보인다).
