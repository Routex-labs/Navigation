...
위 세 단계(경량·의미)는 **건물 안을 보고 있을 때만** 돈다(`indoorContextActive`).
밖에서는 건물 이름과 주변 장소만 찾고 **건물 안 매장은 조회조차 하지 않는다.**

밖에 선 사람이 정하는 것은 "어느 건물로 갈지"이기 때문이다. 매장을 섞으면 "더현대"를
쳤을 때 목록이 그 안의 매장으로 채워져, 정작 고르려던 건물 줄이 뒤로 밀리거나 화면
밖으로 나간다. 화면에서 숨기는 게 아니라 요청 자체를 안 보내므로, 쓰지도 않을 검색이
서버로 나가는 일도 없다. 같은 규칙이 길찾기 시트의 후보 목록
(`MapShellScreen._searchDirectionsCandidates`)에도 걸려 있다.

건물 안 매장으로 가는 길이 막히는 것은 아니다. 건물 줄을 누르면 시트가 "건물 안에서
매장 고르기"를 함께 묻고, 그쪽으로 들어가면 도면 위에서 매장을 직접 누를 수 있다
(아래 절).

밖에서는 실내 검색도 의미 검색도 안 도는 만큼 **결론을 낼 사람이 바깥 조회뿐이다.**
건물 이름 조회와 바깥 조회가 **둘 다** 끝났을 때만 "찾지 못했어요"로 간다
(`_concludeOutdoorOnly`) — 바깥 조회는 기준점이나 TMAP 키가 없으면 그 자리에서 즉시
끝나므로, 한쪽만 보고 결론지으면 없음 문구가 한 번 번쩍인 뒤 건물 줄로 바뀐다.
...
밖에서 검색하면 결과는 **건물 한 줄 + 주변 장소**다(`search_panel.dart`). 건물 줄을
누르면 지도가 그 건물로 이동하고 [`building_sheet.dart`](building_sheet.dart)가 갈 곳을
묻는다. 좌표 하나로 끝내지 않고 묻는 이유는, 같은 검색어("더현대")의 의도가 둘로
갈리기 때문이다.

**길찾기 시트에서 건물을 골라도 같은 시트가 뜬다.** 여기서만 건물 앞 좌표로 안내를
끝내면, 같은 건물을 어디서 골랐느냐에 따라 건물 안 매장까지 갈 수 있기도 하고 없기도 한
앱이 된다. 후보가 건물인지는 `DirectionsCandidate.buildingId`로 가르고, 같은 값이 목록의
아이콘(건물/핀)도 정한다. 그리고 밖에서는 **빈 검색어에 후보를 내지 않는다** — 후보로
남는 것이 건물 한 채뿐이라, 시트가 열리자마자 그 한 줄이 떠 있으면 치지도 않은 답이
정해져 있는 화면이 된다.
...
매장을 고르고 나면 **도면을 접고 야외 안내로 넘어간다**
(`OutdoorMapBodyState.showOutdoorToIndoorRouteTo`). 사용자는 아직 밖에 서 있으므로 지금
필요한 것이 "어느 문으로 들어가는가"이고, 무엇보다 도면을 편 채로 두면 **실내 구간이 영영
안 그려진다** — 실내 구간을 실제 안내로 올리는 트리거가 "건물에 들어가는 순간"인데, 이미
들어와 있으면 그 순간이 다시 오지 않기 때문이다. 접어 두면 문까지의 안내가 야외 지도에
제대로 보이고, 실제로 들어가거나 다시 확대하는 순간 실내 구간이 이어 붙는다.
