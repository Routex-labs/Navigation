...
- **우리가 이미 아는 가게는 뺀다**(`dropPoisCoveredByIndoorStores`). "스타벅스 리저브 /
  B2"(우리)와 "스타벅스 더현대서울(B2)R점"(TMAP)이 나란히 뜨고, 두 줄이 하는 일이
  달랐다 — 아래쪽을 고르면 실내 경로가 안 나온다. 층·노드가 붙어 매장 앞까지
  데려다주는 쪽을 남긴다. 판정은 **건물 안 좌표 + 브랜드(첫 어절) 일치**다. 전체 이름을
  맞추려 하면 영영 안 맞고("스타벅스 리저브" vs "스타벅스 더현대서울(B2)R점"), 건물 밖
  POI는 이름이 비슷해도 건드리지 않는다 — 길 건너 스타벅스는 다른 가게다.
  "건물 안"은 외곽선에서 40 m까지 쳐 준다(`OutdoorMapBodyState.isAtIndoorBuilding`).
  TMAP POI 좌표는 건물 대표점이 아니라 **도로에서 들어오는 접근점**이라 입점 매장도 벽
  바깥 인도에 찍히기 때문이다. 외곽선 안만 인정하면 입점 매장이 하나도 안 합쳐진다.
