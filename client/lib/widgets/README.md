...
- **카테고리를 골라도 카메라는 움직이지 않는다.** 한때 시트가 맨 위 매장을 상위로 올려
  (`onFirstStoreChanged`) 지도를 그 매장으로 옮겼는데, 카테고리를 훑는 동안 지도가 계속
  끌려다녔다. 카테고리는 **강조만** 하고, 카메라는 사용자가 매장을 콕 집었을 때
  (목록 항목 탭·검색 결과 탭)만 움직인다.
