...
/// ## 왜 필요한가
///
/// 원본(다비오) 도면은 한 폴리곤에 매장을 여러 개 붙인다 — 더현대 서울 기준
/// **31개 자리에 91개 매장**이 그렇다(4F 주방용품 10곳, B1 푸드트럭 8곳,
/// 오설록·일상다완 2곳 …). 그 매장들은 `centroid`가 **완전히 같은 값**으로
/// 내려온다.
///
/// 라벨 앵커를 centroid 그대로 쓰면 심볼이 정확히 포개져 MapLibre 충돌 처리가
/// 하나만 남긴다. 화면에는 이름이 하나만 보이고, 나머지 매장은 **존재는 하는데
/// 누를 수가 없다.**
///
/// ## 원본 POI 핀으로 옮기는 것만으로는 안 된다 — 실기기에서 확인한 것
///
/// 처음에는 매장마다 다른 POI 핀 좌표([StorePolygon.entrance])로 앵커를 옮겼다.
/// 지어낸 위치를 쓰지 않는다는 점에서는 맞았지만 **화면에서는 여전히 하나만
/// 보였다.** 오설록·일상다완의 두 핀은 3.6m 떨어져 있는데, 폴리곤(9.6×6.2m)에
/// 맞춰 커진 "오설록" 라벨 하나가 이미 6.8m 폭을 요구한다. 3.6m 간격으로는
/// 겹칠 수밖에 없고, 겹치면 하나가 지워진다.
///
/// **핀 위치는 "서로 겹치지 않는 배치"를 보장하지 않는다.** 그건 원본이 사람
/// 눈으로 그린 지도의 사정이지 우리 라벨 크기와는 무관하다.
///
/// ## 그래서 칸을 나눈다
///
/// 폴리곤이 화면에서 차지하는 세로를 매장 수로 나눠, **한 칸에 하나씩** 위에서
/// 아래로 놓는다. 라벨 크기도 같은 칸 높이에 맞추므로([storeLabelShareCounts]를
/// 쓰는 [FloorPlanView]) 겹칠 수가 없다 — 지어낸 간격이 아니라 폴리곤이 실제로
/// 가진 공간을 나눈 것이다.
///
/// **순서는 원본 핀이 정한다.** 핀을 화면 세로축에 투영해 위에 있던 매장이
/// 위 칸으로 간다. 위치는 우리가 정하고 순서는 원본을 따르는 것이라, 다비오
/// 지도와 위아래가 뒤집히지 않는다. 핀이 없으면 id 순으로 고정한다.
///
/// ## 구현 전에 정한 검증 기준
///
/// | 상황 | 기대 |
