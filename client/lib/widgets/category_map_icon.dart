...
///
/// 매장명 라벨의 `text-size`는 zoom과 폴리곤 크기가 함께 정하지만
/// ([store_label_fit.dart]) 실제로 나오는 값은 9~14px 사이다
/// ([kStoreLabelMinPx]~[kStoreLabelMaxPx]). 아이콘도 같은 구간에서 함께 키워
/// 이름과 한 덩어리로 읽히게 한다. **값을 바꿀 때는 z16 축소와 z20 확대를
/// 반드시 함께 확인하고, 붐비는
/// 층(B1)에서 라벨 개수가 줄지 않았는지도 함께 본다** — 한쪽만 보고 고치면
/// 반대쪽이 회귀한다([kIndoorPoiIconSize] 주석과 같은 함정).
///
/// ## 확대 쪽만 0.20 → 0.24로 키웠다 (2026-08-09)
///
/// "동그란 아이콘이 너무 작아 보인다"는 피드백이다. **양 끝을 다르게 다뤘다** —
/// 심볼 하나가 차지하는 폭은 `아이콘 + 간격 + 이름`이고 아이콘과 글자가 그 예산을
/// 나눠 쓰는데, 같은 차수에서 글자 상한을 18 → 14px로 내린 효과가 zoom 구간마다
/// 다르기 때문이다. z20에서 4글자 이름 기준으로 재 보면:
///
/// | zoom | | 아이콘 | 이름(4글자) | 심볼 폭 |
/// |---|---|---|---|---|
/// | z20 | 이전 | 19.2px | 4 × 18 = 72px | 약 94px |
/// | z20 | 지금 | 23.0px | 4 × 14 = 56px | 약 82px |
/// | z16 | 이전 | 13.4px | 4 × 9 = 36px | 약 51px |
/// | z16 | 0.17로 올렸다면 | 16.3px | 4 × 9 = 36px | 약 54px — **늘어난다** |
///
/// **확대 쪽(z20)은 글자가 상한에 눌려 있어서 상한을 내린 만큼 예산이 남지만,
/// 축소 쪽(z16)은 글자가 이미 하한(9px)이라 더 줄어들 여지가 없다.** 그래서 z16은
/// 0.14 그대로 두고 확대 쪽만 키웠다. 축소 쪽까지 올리면 예전에 0.17~0.25로
/// 키웠다가 B1 한 화면의 매장명이 49개 → 38개로 줄었던 상황을 그대로 되풀이한다.
///
/// 보간이라 실제로는 기본 화면인 z18에서도 16.3 → 18.2px로 커진다.
/// **그래도 실기기에서 라벨 개수는 확인해야 한다** — 위 계산은 4글자 기준이고
/// 긴 이름에서는 여유가 적다.
