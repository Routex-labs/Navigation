...
abstract class OutdoorPoiRepository {
  ...
  /// [keyword]로 장소를 찾는다. [center]는 **거리순 정렬의 기준점**이다.
  ///
  /// [radiusMeters]가 0이면 반경으로 자르지 않는다(기본값). 이게 중요하다 —
  /// 반경을 걸면 "집에서 더현대 서울까지" 같은 검색이 통째로 죽는다. 사용자가
  /// 지금 서 있는 곳에서 20 km 떨어진 목적지는 반경 밖이라 결과가 아예 비고,
  /// 화면에는 "찾지 못했어요"가 뜬다 — 목적지가 멀다는 것과 그런 곳이 없다는
  /// 것은 전혀 다른 이야기인데 사용자는 후자로 읽는다.
  ///
  /// 대신 [center] 기준 **거리순**으로 정렬해 가까운 후보가 위로 온다. 같은
  /// 브랜드의 다른 지역 지점이 첫 줄에 오는 것은 그렇게 막는다.
  ///
