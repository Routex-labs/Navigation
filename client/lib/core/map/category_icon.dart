...
/// 대분류 색.
///
/// 패션·음식점·카페·뷰티·식품관 다섯 갈래는 **지정된 Pantone 값**을 쓴다. 임의로
/// 밝기를 올리거나 내리면 지정색이 아니게 되므로, 색을 조정해야 할 일이 생기면
/// 색상값이 아니라 아래 [_colorByCategory]를 쓰는 쪽(원 배경·면 채우기)의 대비를
/// 손본다.
///
///  - 패션 → PANTONE 536 C
///  - 음식점 → PANTONE 7612 C
...
/// 원색(indigo·teal·blueGrey·pink)을 그대로 썼는데, 지정 다섯 색만 뮤트 파스텔이고
/// 넷만 원색이라 화면에서 두 팔레트가 섞여 보였다. 그래서 넷을 지정색의 톤 대역
/// (HSL 채도 21~73%·명도 61~81%, 상대 휘도 0.38~0.52)에 맞춘 파스텔로 눌렀다 —
/// 흰 글리프(지도 배지·선택 chip 글자)가 올라가므로 지정색보다 밝게 잡지 않았다.
///
///  - 서비스 → 옛 indigo의 색상각(≈232°)을 지킨 뮤트 페리윙클
///  - 리빙 → 옛 teal의 색상각(≈173°)을 지킨 뮤트 틸 — 식품관(≈149°)과는
///    청록 쪽으로 벌어져 있어 붙어 있어도 갈린다
///  - 편의시설 → 뮤트 회청. 패션(215°·S26%)과 헷갈리지 않게 채도를 더
///    낮췄다(S15%) — 아홉 색 중 가장 중성이라 "매장 아닌 시설"로 읽힌다
///  - 키즈 → 파스텔 라일락(≈284°). 옛 분홍(340°)은 뷰티(349°)와 같은
///    색상축이라, 아홉 색이 비워 둔 보라축으로 옮겼다
///
/// **음식점·카페·식품관은 한 갈래에서 나뉜 만큼 서로 붙어 있으면 구분이 안 된다.**
/// 예전에는 주황(음식)/갈색(커피)/초록(식재료)으로 연상이 다른 색을 골라 벌려
/// 뒀는데, 지정색은 셋 다 저채도라 그때만큼 멀지 않다. 초록만은 여전히 따뜻한
/// 두 색과 색상환에서 떨어져 있어 셋을 가르는 축으로 남는다.
