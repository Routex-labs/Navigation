...
///
/// 평소에는 칠하지 않는다 — 항상 칠하면 도면 전체가 색 밭이 되어 지금 무엇이
/// 선택됐는지가 오히려 안 읽힌다. 색은 chip·시트·라벨 아이콘이 쓰는 것과 **같은
/// 표**([categoryColorFor])이고, 면은 옅게(글자가 읽히도록) 테두리는 원본 색으로
/// 둔다.
///
/// 카테고리마다 레이어를 얹지 않고 **강조 레이어 하나**의 색만 데이터 기반
/// 표현식으로 바꾼다. `match`의 label 자리에는 문자열만 넣는다 — 배열을 label로
/// 쓰면 MapLibre GL Native에서 조용히 매치 0건이 되는 경로가 있다.
///
/// 그 밖의 근거는 `docs/client/map-style-rules.md`.
library;
