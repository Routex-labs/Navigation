...
void main() {
  ...
  test('면 색은 매장명이 읽힐 만큼 밝다', () {
    // 라벨은 #444846(휘도 약 0.06)이다. 가장 어두운 대분류(서비스·리빙)까지
    // 포함해 면이 충분히 밝아야 이름이 먼저 읽힌다. 이 임계값을 낮추려면
    // 실기기에서 서비스·리빙 매장의 이름 가독성부터 확인해야 한다.
  ...
  test('리빙은 초록 계열 한 가지로 면·선이 함께 간다', () {
    // 사용자가 지목한 대분류다. 시트·chip에서 쓰는 색(#00897B)이 그대로
    // 지도 경계선이 되어야 "시트에서 본 초록 = 지도의 초록"이 성립한다.
    expect(categoryOutlineHex('리빙'), '#00897B');
    expect(categoryFillTintHex('리빙'), '#D1EAE7');
  });
