...
관계**였기 때문이다(추정: 원본 이미지 정합 과정이 진짜 similarity가 아니었을
...
``fit_affine_transform``을 직접 쓰지 말고 ``fit_wgs84_transform``을 써야 한다.
"""
...
class GeoTransform:
    ...
    def apply(self, x_m: float, y_m: float) -> tuple[float, float]:
        ...
        ``apply_uv``가 등방 공간의 (u, v)를 돌려주면, u를 lng_scale로 나눠
        진짜 경도로 되돌린다(``fit_wgs84_transform`` 참고).
        """
...
def fit_affine_transform(pairs: Iterable[PointPair]) -> GeoTransform:
    ...
    둘 다 같은 물리적 스케일 단위). (u, v)가 (lng, lat)이면 ``fit_wgs84_transform``을
    ...
    Raises:
        ValueError: 대응점이 3개 미만이면(축별 스케일이 다를 수 있는 6개
            미지수를 유일하게 결정하려면 최소 3개의 비공선 점이 필요하다).
    """
    pairs = list(pairs)
    if len(pairs) < 3:
...
def fit_wgs84_transform(pairs: Iterable[PointPair]) -> GeoTransform:
    ...
    만든 뒤) 피팅하고, 그 lng_scale을 결과 ``GeoTransform``에 담아 돌려준다.
    ``apply()``가 이 값을 이용해 최종 결과를 다시 진짜 (lat, lng)로 되돌린다.

    Raises:
        ValueError: 대응점이 3개 미만이면.
    """
    pairs = list(pairs)
    if len(pairs) < 3:
        raise ValueError("affine 변환을 피팅하려면 대응점이 3개 이상 필요합니다.")
