...
| 어디 | 무엇 | 분량 |
|---|---|---|
| 파일 머리 | 이 파일이 무엇인지 한 문장 + 지켜야 할 규칙 요약 + 문서 경로 한 줄 | **8줄 이하**(CI가 검사한다) |
| 선언 위 `///` | 호출 계약 — 입력 범위, null의 뜻, 실패 조건 | **한 덩어리 13줄 이하**(CI가 검사한다) |
...
- 두 상한은 `client/test/lib_header_comment_length_test.dart`가 지킨다. 그 테스트 주석에 각 값의 근거가 있다
  (Flutter SDK 실측 비교, 그리고 "상한은 지금 최대치 바로 위에 둬야 다시 자라지 않는다").
- **길어서 상한에 걸리면 지우지 말고 옮긴다.** 실측 로그·버린 대안·검증 기준 표는 `docs/`에 두고 코드에는
  경로 한 줄만 남긴다. 옮길 자리가 없으면 그 문서를 새로 만든다.
