...
- **진입점은 맨 아래 탭 줄 하나이고, 건물 안에서만 눌린다.** 밖에서는 흐리게 둔다 — 자리를
  빼면 바닥이라는 약속이 깨지고, 그렇다고 열어 주면 몇 킬로 떨어진 데서 "오늘의 이벤트"가
  **지금 있는 동네 이야기로 읽힌다.** 이 앱의 행사는 더현대 서울 한 건물 것뿐이다.
  지도 위 pill은 걷어냈다.
