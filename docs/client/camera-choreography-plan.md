...
| 자동 전환 페이드인 = 사람 조작의 **2배** | 훑기는 스스로 일으킨 조작이라 빠를수록 좋지만, 자동 전환은 요청하지 않은 사건이라 "지금 층이 바뀌고 있다"를 알아챌 시간이 필요하다. 실기기에서 기존 연출은 "너무 빠르다"는 지적을 받았고 그때 지도가 갈리는 것이 전환이 아니라 **깜빡임**으로 읽혔다. 마커 활강(2.4초)보다는 짧게 둔다 — 도면이 먼저 자리를 잡아야 사용자가 새 층에서 자기 위치를 찾을 시간이 남는다 |
