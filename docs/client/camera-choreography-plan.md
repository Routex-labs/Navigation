...
| Q2 | 층 전환에서 카메라를 언제 움직일까 | **마커와 같은 시간 동안.** 처음에는 스크림 뒤에서 조용히 옮겼는데, 스크림 자체가 깜빡임으로 읽혀 걷어냈다([4.5](#45-층-전환은-덮지-않고-이어-붙인다)) |
...
### 4.5 층 전환은 덮지 않고 이어 붙인다

한때 층이 바뀌는 구간을 불투명 스크림으로 덮고 그 뒤에서 카메라를 `Duration.zero`로
옮겼다. 사용자가 움직임을 못 보게 해서 "순간이동"으로 읽히지 않게 하려던 것인데,
실기기에서는 스크림 자체가 **깜빡임**으로 보였다 — 지도가 하얗게 날아가고 그 사이
마커가 사라진다. 지금은 덮지 않는다. 도면은 크로스페이드되고, 마커는 탑승 노드에서
하차 노드로 흐르며(`EscalatorGlide`), 카메라는 **같은 시간 동안** 하차 지점으로
이동한다. 셋이 같은 시간 축 위에 있어야 하나의 전환으로 읽힌다.
