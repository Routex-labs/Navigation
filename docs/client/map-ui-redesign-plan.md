...
끌어올리기 위한 작업 계획. 헤더 개선 1건은 완료됐고, 이번 차수에서 5건(2·1·3·4·6)을
한 브랜치에서 진행한다. 나머지 2건(5·7)은 보류다.

- 설명·주석·커밋은 한국어, 코드·식별자는 영어 (AGENTS.md 규칙).
- 이 문서는 **무엇을 왜 바꾸는지**와 **무엇이 충족되면 맞다고 볼지**를 담는다.
  이번 차수는 브랜치 하나(`feat/map-ui-refresh`)에서 항목별로 커밋을 나눠 진행하고,
  각 항목의 검증 기준을 통과한 뒤 다음 항목으로 넘어간다.
...
| # | 항목 | 상태 | 비고 |
|---|---|---|---|
| 0 | 상세 헤더 아이콘·층 표기 | **완료** | `feat/place-detail-header-icon`, PR #71 머지 |
| 2 | 폰트 통일 | **이번 차수** | 가장 먼저 — 전체 텍스트 metrics가 바뀐다 |
| 1 | 지도 오버레이 겹침·여백 | **이번 차수** | 상단 바 높이가 폰트에 딸려 바뀐다 |
| 3 | 지도 팔레트 대비 | **이번 차수** | 4·6의 색을 이 배경 위에서 고른다 |
| 4 | 마커·시설 아이콘 | **이번 차수** | |
| 6 | 경로선 스타일 | **이번 차수** | |
| 5 | 매장 라벨 시스템 | **보류** | 백엔드 선행 필요. 이번 차수에서 제외 |
| 7 | 길찾기 입력 구조 | **보류** | 시트 chain 재배선으로 범위가 큼. 이번 차수에서 제외 |
| — | 영업시간·연락처 | 보류 | 데이터 출처 결정 필요 |
| — | 도착지 이름 라벨 | 보류 | 4 이후 |

### 이번 차수 — `feat/map-ui-refresh`

**2 · 1 · 3 · 4 · 6을 브랜치 하나에서 순서대로 진행한다.** 항목마다 커밋을 나누되
브랜치는 쪼개지 않는다. 다섯 항목이 모두 지도 화면의 같은 표면을 건드려서, 따로
머지하면 중간 상태에서 서로 어긋난 화면이 한 번씩 나온다.

```mermaid
graph LR
    F["2. 폰트 통일"] --> O["1. 오버레이 레이아웃"]
    F --> P["3. 지도 팔레트"]
    P --> M["4. 마커·시설 아이콘"]
    P --> R["6. 경로선"]
```

- **2 → 1**: 폰트가 바뀌면 `MapTopBar`의 높이가 달라진다. 레이아웃을 먼저 잡으면
  폰트 교체 후 다시 봐야 한다.
- **3 → 4, 6**: 마커 테두리 색과 경로선 casing 색은 **배경 대비를 보고 고르는 값**이다.
  배경을 나중에 바꾸면 두 색을 다시 정해야 한다.
- 4와 6은 서로 독립이라 순서 자유.

### 보류 항목을 뺀 이유

- **5번(매장 라벨)** — `tiling.py`에 `category`를 싣고 타일 캐시를 비우는 백엔드 작업이
  선행돼야 한다. 클라이언트 차수와 성격이 다르고, `feature/store-icons` 브랜치 이력도
  확인이 필요하다(아래 참조).
- **7번(길찾기 입력)** — `directions_sheet.dart` 제거 + 시트 chain 계약 재배선이라
  상세 시트·카테고리 시트까지 영향이 간다. 표현 계층만 만지는 이번 차수와 섞으면
  회귀 범위가 흐려진다.

두 항목의 설계는 아래 본문에 그대로 남겨 둔다 — 착수할 때 이 문서에서 이어받는다.

### 진행 확인 (2026-08-02, main `8ca561b`)

0번 머지 이후 main에 커밋 20건이 들어왔지만 **전부 검색·질의(query/intent/facet)
작업**이고, 1~7번은 **하나도 착수되지 않았다.** 아래 값이 모두 그대로다.

| # | 확인한 것 | 파일 |
|---|---|---|
| 1 | `top: 78` ×2, `top: 128` ×2 | `map_shell_screen.dart` |
| 2 | `theme.fontFamily` 없음 · `pubspec` fonts 주석 · 백엔드 폰트 `Regular` 하나 | — |
| 3 | `#F3F1EF` / `#D8D4D1` | `floor_plan_view.dart`, `indoor_overlay_layers.dart` |
| 4 | 야외 핀 단색 · 아이콘 최대 `0.6` · 시설 라벨 `textSize: 11` | `outdoor_map_screen.dart`, `indoor_overlay_layers.dart` |
| 5 | 타일 properties가 `{id, name, kind}` | `backend/app/geo/tiling.py` |
| 6 | 흰 casing `#FFFFFF` 8px · 실내 `lineOpacity: 0.6` | `outdoor_map_screen.dart`, `floor_plan_view.dart` |
| 7 | `directions_sheet.dart` 존재 · `resizeToAvoidBottomInset: false` | `map_shell_screen.dart` |

⚠️ **`feature/store-icons` 원격 브랜치가 삭제된 이력이 있다.** 이름이 5번과 겹치는데
PR 목록에도, main에도 흔적이 없다. 누가 착수했다 접었거나 로컬에만 남겨뒀을 수 있으니
**5번을 시작하기 전에 팀에 확인**한다.
...
- 백엔드: fontstack에 Bold 추가 → 지도 라벨 굵기 위계 확보 (보류 중인 5번의 선행 조건)
...
## 1. 지도 오버레이 겹침·여백 <sub>이번 차수 · 2순위</sub>
...
**테두리를 흰색으로 하면 안 되는 이유**는 6번 경로선에서 짚은 것과 같다. 실내 도면
...
- 이번 차수는 `feat/map-ui-refresh` **한 브랜치**에서 진행한다. 다섯 항목이 지도 화면의
  같은 표면을 건드려서, 따로 머지하면 중간 상태에서 서로 어긋난 화면이 나온다.
- 대신 **항목마다 커밋을 나눈다.** 한 커밋에 두 항목을 섞지 않는다.
