...
- 글씨는 **흰색**으로 그 안에 넣는다.
- 마커 실루엣 전체에 **흰 아웃라인**을 두른다. 지도 배경이 밝든 어둡든 마커가 배경에서
  떠 보이게 만드는 건 이 아웃라인이다.

→ **이 규칙을 따른다.** 흰 원을 되살리는 방향은 버린다 — 속을 비우면 마커의 색 면적이
줄어 멀리서 눈에 덜 띄고, 아웃라인 없이 흰 배경 도면 위에 놓이면 경계가 사라진다.

```
핀 머리   빨강(AppColors.dest) 채움, 반지름 24
꼬리      머리에서 가늘게 떨어지는 곡선. 현재 핀(직선 탄젠트)보다 얇고 길게
글씨      흰색 "도착". 머리 안에서 숨 쉬도록 현재보다 키운다
아웃라인  흰색 3px, 머리와 꼬리를 포함한 실루엣 전체
```
