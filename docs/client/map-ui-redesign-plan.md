...
끌어올리기 위한 작업 계획. 헤더 개선 1건은 완료됐고, 이번 차수에서 1~5번을 진행한다.
6·7번은 보류다.
...
| 1 | 폰트 통일 | **이번 차수** | 가장 먼저 — 전체 텍스트 metrics가 바뀐다 |
| 2 | 지도 오버레이 겹침·여백 | **이번 차수** | 상단 바 높이가 폰트에 딸려 바뀐다 |
| 3 | 지도 팔레트 대비 | **이번 차수** | 4·5의 색을 이 배경 위에서 고른다 |
| 4 | 마커·시설 아이콘 | **이번 차수** | |
| 5 | 경로선 스타일 | **이번 차수** | |
| 6 | 매장 라벨 시스템 | **보류** | 백엔드 선행 필요 |
| 7 | 길찾기 입력 구조 | **보류** | 시트 chain 재배선으로 범위가 큼 |
| — | 영업시간·연락처 | 보류 | 데이터 출처 결정 필요 |
| — | 도착지 이름 라벨 | 보류 | 4 이후 |

### 이번 차수 (1~5번)

**1번부터 5번까지 순서대로 진행한다.** 번호가 곧 실행 순서다.
항목마다 커밋을 나눈다.
...
- **6번(매장 라벨)** — `tiling.py`에 `category`를 싣고 타일 캐시를 비우는 백엔드 작업이
...
| # | 확인한 것 | 파일 |
|---|---|---|
| 1 | `theme.fontFamily` 없음 · `pubspec` fonts 주석 · 백엔드 폰트 `Regular` 하나 | — |
| 2 | `top: 78` ×2, `top: 128` ×2 | `map_shell_screen.dart` |
...
| 5 | 흰 casing `#FFFFFF` 8px · 실내 `lineOpacity: 0.6` | `outdoor_map_screen.dart`, `floor_plan_view.dart` |
| 6 | 타일 properties가 `{id, name, kind}` | `backend/app/geo/tiling.py` |
