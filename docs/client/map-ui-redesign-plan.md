...
| — | 영업시간·연락처 | **부분 해소** | 소개 영상용 1개 매장에만 `demoInfo`로 열림. 전 매장 확대는 여전히 보류 |
| — | 도착지 이름 라벨 | 보류 | 4 이후 |
...
⚠️ **가드 테스트의 기준을 바꿨다.** `tests/unit_test/category_map_icon_test.dart`가
...
- [x] 목표 줄 수가 실제로 나오는 폭을 고른다
      (`tests/unit_test/store_label_fit_test.dart` — `조 말론 런던`을 회귀 케이스로 박았다)
- [x] 아이콘을 키운 만큼 심볼 폭이 넓어지지 않는다
      (`tests/unit_test/category_map_icon_test.dart`)
...
### 영업시간·연락처 — 한 매장만 열렸고, 확대는 여전히 보류다

> **2026-08-10 갱신.** 소개 영상에 나오는 스타벅스 리저브 한 곳에만 영업시간·대표번호가
> 올라갔다. `demoInfo` 필드 + `_schema.json`의 `demo_allowlist`에 id가 있는 매장만 쓸 수
> 있고, 항목마다 `source`(그 값이 적혀 있던 http(s) 페이지)와 `confirmed_at`이 필수다.
> 화면에는 확인일을 함께 그린다. 경위와 방어 장치는
> [9-1 D2″](../backend/place-detail/place-detail-interface.md).
>
> **아래 선택지 논의는 그대로 유효하다.** 열린 것은 "한 매장에 문자열로 적는 것"까지이고,
> 아래가 다루는 **전 매장 확대와 "지금 영업 중" 판정은 하나도 해결되지 않았다.** 그래서
> 이 절을 지우지 않고 남긴다.
...
| B | **`hours`·`phone` 전용 필드로 승격** + `source` 필수 | **권장 — 전 매장 확대를 하려면 여전히 이것** |
| C | "데모 데이터" 배지 달고 그냥 표시 | 비추 — 결국 거짓이 화면에 뜬다 |
| D | **`demoInfo` + 매장 허용 목록** | **채택(2026-08-10)**. 단 소개 영상용 1곳 한정이고, 확대하려면 B가 필요하다 |
...
7612 C·7508 C·700 C·557 C), 나머지는 Material 팔레트다.
