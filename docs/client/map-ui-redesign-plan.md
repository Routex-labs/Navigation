...
| 7 | 길찾기 입력 구조 | **완료(2026-08-10)** | 상단 바가 출발/도착 두 칸이 된다 |
...
**7번(길찾기 입력)은 끝났다(2026-08-10).** 한 번 돌아왔다 — 2026-08-09에 전용 길찾기
화면으로 갔다가, 제안대로 **상단 바에서 아래로 펼쳐지는 패널**로 되돌렸다. 이동 수단
줄을 입력 위가 아니라 **아래**에 두면 상단 바 안에 다 들어간다. 자세한 내용은 아래 7번 본문.
...
| 7 | **완료(2026-08-10)** — `directions_sheet.dart` 제거, 상단 바가 출발/도착 두 칸이 된다. `resizeToAvoidBottomInset: false`는 상단 검색 패널이 계속 써서 그대로 | `widgets/map_top_bar.dart`, `widgets/route_field_results.dart`, `map_shell_screen.dart` |
...
**제안대로 상단 바다.** 길찾기를 시작하면 상단 바가 출발/도착 두 칸이 되고, 그 아래에
이동 수단 줄이 붙고, 후보 목록만 그 밑으로 펼쳐진다. 지도는 계속 보인다.

2026-08-09에는 한 번 **전용 화면**(`screens/route_planner/`)으로 갔었다. 이동 수단을
입력보다 **위에** 놓아 "어떻게 갈지를 먼저 정하는" 흐름을 만들려 했는데, 그 줄까지 상단
바에 넣으면 높이가 안 나온다는 판단이었다. 실제로 써 보니 대가가 더 컸다 — 목적지를
고치려면 보던 지도를 잃고 화면을 다시 열어야 했고, 상단에는 값만 적힌 요약 행이 남아
**방금 누른 칸과 실제로 치는 칸이 서로 다른 화면에** 있었다.

수단 줄을 입력 **아래**로 내리면 그 높이 문제가 사라진다. 순서도 "어디로 갈지 → 어떻게
갈지"가 되는데, 어차피 도착지를 정하면 거리를 보고 수단이 자동으로 정해지므로
(가까우면 도보, 멀면 대중교통) 그 줄은 대개 고르는 곳이 아니라 **지금 무엇으로 안내
중인지 보여주고 바꾸는 곳**이다.

- 자동차: 경로를 그리고 **곧바로 카메라가 현재 위치를 따라간다.** 운전 중에는 화면을
  손으로 옮길 수 없으므로 지도가 따라와야 하고, 자동차를 고른 것 자체가 "지금
  출발한다"는 뜻이라 "안내시작"을 한 번 더 누르게 하지 않는다.
- 도보: 경로만 그린다. 남은 거리·시간은 지도 위 ETA 카드가 말한다.
- 대중교통: 하단에서 후보 목록 시트가 올라오고, 하나를 고르면 그 경로를 지도에 그리고
  요약 카드로 바꾼다.

셋 다 그만두는 조작은 두 개뿐이다 — 상단 바의 **X**와 하단 카드의 **안내 종료**. 둘은
같은 일을 한다(경로선·도착 핀·상단 두 칸이 함께 사라진다).

지도를 새로 만들지 않고 이미 떠 있는 야외 지도에 그린다 — 이유는
[`lib/screens/README.md`](../../client/lib/screens/README.md#길찾기는-화면이-아니라-상단-바다).

![길찾기 입력 구조 변경 전후](images/07-directions.svg)

### 위험

범위가 가장 컸다. `directions_sheet.dart` 제거 + 시트 chain 계약
(`StoreInfoAction`, `_runSheetChain`) 재배선이라 상세 시트·카테고리 시트까지 영향이
갔다. 결과적으로 시트 chain은 건드리지 않았다 — 상세 시트의 "도착"은 지금도
`_startRoute`로 곧장 간다.

### 검증 기준

- [x] 키보드가 올라와도 입력 필드가 가려지지 않음 — 입력이 화면 맨 위로 올라갔다
- [ ] 출발·도착을 서로 바꾸는 동작이 유지됨 — **못 했다.** 전용 화면의 swap 버튼이
      상단 바로 돌아오면서 자리를 잃었다. 두 칸을 직접 고쳐 칠 수 있으니 막힌 흐름은
      아니지만, "왔던 길 되돌아가기"는 다시 넣을 값어치가 있다
...
- [x] 뒤로가기로 draft가 유실되지 않음 — 두 칸의 상태를 `MapShellScreen`이 소유하고
      뒤로가기는 입력만 접는다(한 번 더 눌러야 길찾기가 끝난다)
