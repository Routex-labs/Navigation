...
- 자동차: 고르는 즉시 지도에 경로 하나 + 아래 요약 카드("안내시작"). 그 버튼은
  카메라를 현재 위치로 확대해 **따라가기 시작한다** — 운전 중에는 화면을 손으로
  옮길 수 없으므로 지도가 따라와야 한다.
- 도보: 경로만 그리고 화면을 닫는다. **"안내시작"이 없다** — 도보에는 시작할 안내가
  없고(지도 위 ETA 카드가 그대로 이어받는다), 버튼을 한 번 더 누르게 하면 누르기
  전과 후의 화면이 사실상 같아 그 버튼이 무엇을 켰는지 알 수 없다.
