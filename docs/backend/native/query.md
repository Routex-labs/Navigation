...
- 요청: `{ "text": str, "building_id": str, "current_floor": str | null }`
  - `current_floor`는 선택 사항이다(사람이 보는 층 라벨, 예: "B2"). 값이 있으면 해당 층에서만 매칭해 현재 층의 화장실·엘리베이터·에스컬레이터를 바로 찾는다. 생략하면 기존처럼 건물 전체에서 검색한다.
...
- 요청: `{ "text": str, "building_id": str, "current_floor": str | null }`
  - `current_floor`를 주면 `floors`도 현재 층 하나만 반환한다. 생략하면 대상이 존재하는 모든 층을 반환한다.
