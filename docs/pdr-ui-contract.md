...
| `runtimeStatuses` | `Stream<PdrRuntimeStatus>` | iOS 센서 준비·실행·pause·오류 상태 변화 |
| `currentRuntimeStatus` | `PdrRuntimeStatus` | 최근 센서 pipeline 실행 상태 |
...
`startGuidance`, `stopGuidance`, `changeFloor`는 플랫폼 센서 호출을 포함하므로 반드시 `await`한다.
호출 실패는 처리되지 않은 플랫폼 예외로 새지 않고 §5 runtime status로 노출된다.
...
## 5. 센서 실행 상태 — `PdrRuntimeStatus`

| state | 의미 |
|---|---|
| `idle` | 안내와 센서 세션이 꺼져 있음 |
| `starting` | EventChannel을 열고 첫 native 이벤트를 기다리는 중 |
| `running` | native 이벤트를 받아 PDR 세션에 전달하는 중 |
| `paused` | 앱 background로 tracking과 native 센서를 멈춘 상태 |
| `stopping` | 명시적인 안내 종료 처리 중 |
| `degraded` | 권한·센서·채널 오류로 정상 위치 갱신을 보장할 수 없음 |

`warnings`는 사용자 문구가 아니라 UI와 telemetry가 해석할 안정된 식별자다.

- `sensorStartFailed`: 최초 센서 시작 실패
- `sensorStreamError`: EventChannel 수신 오류
- `sensorStopFailed`: background 센서 정지 실패
- `sensorResumeFailed`: foreground 센서 재시작 실패
- `pedometerResetFailed`: 층 변경 pedometer reset 실패

`degraded`이면 오류 이후 방출되는 `PdrSnapshot.quality.state`도 `degraded`이고 같은 warning을
포함한다. 세션 시작 전처럼 snapshot이 아직 없을 때는 `currentRuntimeStatus`를 직접 사용한다.
