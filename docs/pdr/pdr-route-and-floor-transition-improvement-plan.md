...
> 마지막 레이어에 있다. veil이 덮는 구간은 계획보다 넓다 — 도면 교체 프레임이 아니라 **걸음이
> 멈추는 순간부터 하차까지**다. 덮개 뒤에서 도면 크로스페이드와 마커 활강이 돌아, 걷힐 때
> 새 층과 하차 지점이 이미 자리를 잡고 있다(임계값·문구·연출은
...
- **덮는 구간은 걸음 정지 ~ 하차**다. 계획의 "도면 swap 구간만"에서 넓혔다 — 그 앞뒤로
  걸음이 멈춘 마커가 지도에 굳은 채 서 있어 "위치가 고장 났다"로 읽혔다.
...
- [x] 걸음이 멈추는 순간부터 하차까지 veil이 덮고 뒤쪽 입력을 막는다.
